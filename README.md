...
- **広告**
  - Google AdSense などの軽量広告を導入
  - 画面下部や非アクティブ領域に表示してユーザー体験を阻害しない形を想定
- **アフィリエイト**
  - 宿泊予約サイト（例：楽天トラベル、じゃらん、Booking.com など）や
    観光スポットのチケット販売サービスと連携
  - スポット詳細画面や宿泊関連の提案箇所にリンクを配置

### 🔜 ユーザー定着後

- **サブスクリプション（Stripe 導入予定）**
  - 月額 300〜500 円を想定
  - 無料版との差別化として以下の機能を有料化候補にする
    - 保存可能プラン数の無制限化（無料は上限あり）
    - 移動モードの拡張（電車／車対応）
    - リマインダー通知（旅行前日アラートなど）
    - 共同編集機能（グループ全員での同時編集）
- **自然なアップセルの流れ**
  - 保存上限に達した際に「プレミアムにアップグレード」導線
  - 移動モードを切り替えようとした際にプレミアム案内を提示

### 🚀 グロース期

- **プレミアムプラン拡張**
  - 詳細な旅程分析（移動時間合計、滞在時間計算）
  - ユーザーの好みに基づくおすすめルート提案
  - SNS シェア用の画像生成、QR コード共有
- **広告／アフィリエイトの最適化**
